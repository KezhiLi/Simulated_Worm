...
%     for ii = 4100:1:4700   %size(Xrecon,1) % only plotting every 10 frames to speed up movie
% %   
% %         subplot(5,4,[13,14])
...
%  midbody_bends = worm.locomotion.bends.midbody.amplitude(round(0.1*midbody_bends_length):3:end);
 ...
 idx_timeseries = size(fea_timeseries_subs,2)- size(c_L{1},2)+1;
 fea_timeseries_show =  fea_timeseries_subs(:,idx_timeseries:end);
...
for ii = 1:4;
    ...
    R_largeEntry{ii}(abs(R{ii})>0.6) = R{ii}(abs(R{ii})>0.6);
...
for ii = 1:4;
    ...
    R2_largeEntry{ii}(abs(R2{ii})>0.6) = R2{ii}(abs(R2{ii})>0.6);
  %  R_largeEntry{ii}(1:56,1:56)=0;
  %  R_largeEntry{ii}(57:end,57:end)=0;
